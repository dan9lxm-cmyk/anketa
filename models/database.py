...
# Импортируем все константы из config
try:
    from config import (
        APPLICATIONS_FILE, SETTINGS_FILE, ACTIVE_CHATS_FILE,
        BLOCKED_USERS_FILE, TEMP_CHANNELS_FILE, CHAT_HISTORY_FILE,
        POLLS_FILE, MAX_CHATS_PER_USER
    )
except ImportError:
    # Значения по умолчанию, если импорт не работает
    ...
    print("⚠️ Используются значения по умолчанию для файлов данных")
...
class ApplicationsDB(BaseDatabase):
    ...
    def save_application(self, app_id: str, user_id: str, username: str,
                         content: Dict, approved_by: Optional[str] = None,
                         approved_at: Optional[str] = None) -> bool:
        data = self.load()
...
class PollsDB(BaseDatabase):
    ...
    def create_poll(self, poll_id: str, creator_id: str, creator_name: str,
                    ...
                    voting_notify_time: int, options: List[str]) -> Dict:
        data = self.load()
        poll_data = {
...
class ChatsDB(BaseDatabase):
    ...
    def create_chat(self, chat_id: str, application_id: str,
