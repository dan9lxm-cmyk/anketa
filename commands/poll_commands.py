...
class PollCommands(commands.Cog):
    ...
    async def poll_status(self, ctx):
        ...
        try:
            ...
            poll_id = self.poll_manager.db.load().get("active_poll")
            active = self.poll_manager.db.get_poll(poll_id) if poll_id else None
            ...
            if active.get("is_poll_active", False):
                ...
                remaining = max(0, active["poll_duration"] - elapsed)
                embed.add_field(
                ...
                remaining = max(0, active["voting_duration"] - elapsed)
                embed.add_field(
                    name="⏱ Осталось времени",
                    value=f"{remaining} секунд",
                    inline=True
