...
class UserCommands(commands.Cog):
    ...
    async def update_roles(self, ctx):
        ...
        try:
            ...
            # Проверяем, есть ли роль новичка
            has_newbie = False
            ...
            embed = discord.Embed(
                ...
                description=f"{ctx.author.mention}, вы можете изменить свои роли!\n\n"
            ...
            # Создаем view для изменения ролей
            view = ChangeRolesView()
    ...
    async def list_chats(self, ctx):
        ...
        try:
            ...
            for i, (chat_id, chat) in enumerate(user_chats[:10], 1):
                ...
                app_id = chat.get("application_id")
                app_data = self.apps_db.get(app_id) if app_id else {}
...
class ChangeRolesView(discord.ui.View):
    """Вид для изменения ролей"""
    ...
    async def change_games(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message("🎮 Функция смены игр в разработке", ephemeral=True)
    
    @discord.ui.button(label='👤 Указать пол', style=discord.ButtonStyle.secondary, custom_id='add_gender')
    async def add_gender(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message("👤 Функция указания пола в разработке", ephemeral=True)
    
    @discord.ui.button(label='🎂 Указать возраст', style=discord.ButtonStyle.secondary, custom_id='add_age')
    async def add_age(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message("🎂 Функция указания возраста в разработке", ephemeral=True)